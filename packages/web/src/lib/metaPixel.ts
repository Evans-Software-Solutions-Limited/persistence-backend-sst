...
/**
 * Inject the Meta Pixel base code + `fbq('init', <id>)`. No-op when
 * `VITE_META_PIXEL_ID` is unset, or when the pixel has already been
 * injected (idempotent — safe to call more than once).
 */
...
/** Fire the standard `PageView` event. No-op when the pixel isn't loaded. */
 ...
 * the server-side CAPI `Lead` sharing the same id (spec-30 R3.2). No-op when
