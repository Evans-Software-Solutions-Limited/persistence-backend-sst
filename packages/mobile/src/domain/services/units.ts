 ...
 * `2 cups`) and links to a Food that carries its macros per `servingSize` of
 * `servingUnit` (e.g. per `100 g`). To sum macros we need the factor "how
 * many of the food's servings does this ingredient represent".
 *
 ...
 * food"), preserving prior behaviour. The whole-recipe AI estimate
 * (`useEstimateRecipe`) is the escape hatch for imports whose ingredient units
 * we can't reconcile.
 *
 * Mirrors the backend helper EXACTLY:
 * `microservices/core/src/application/recipes/services/units.ts`.
 */
