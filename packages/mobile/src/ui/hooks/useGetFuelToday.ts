...
export function useGetFuelToday(date: string, enabled = true): FuelTodayState {
  ...
  useEffect(() => {
    ...
    // failed fetch (e.g. the 503s launch fan-out produces) leaves
    // `autoRefreshedRef` set forever: closing and reopening the sheet would
    // find it already armed and never retry, silently stranding the caller
    // on whatever fallback it renders for a null `data`. Unlike
