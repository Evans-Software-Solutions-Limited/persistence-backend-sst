...
### Abuse ceiling (DECIDED — Brad 2026-07-05, with a profit buffer; amends cross-cuts § 4.3)

Server-side **daily per-user inference ceilings, per endpoint**: photo **12/day**, free-text **30/day** (env-tunable `AI_PHOTO_DAILY_LIMIT` / `AI_TEXT_DAILY_LIMIT`). Over the limit → `429 { error: 'ai_daily_limit' }`; mobile shows "Daily AI limit reached — it resets tomorrow." This is NOT a product quota — no genuine user hits 12 photo estimates/day — it is a cost backstop sized so worst-case per-user AI spend (~£7.30/mo at EU Bedrock prices) stays well under the £12.99 premium price, per Brad's requirement that AI cost can never equal what a user pays. Counter = today's (UTC) `ai_usage_log` rows for that endpoint; **only actual inferences are logged/counted** (pre-model rejections — 402/400/413/429 — write no row and consume no quota; 422/503 model failures DID cost money and do count). Best-effort under concurrency (backstop, not exact). The buffer also pre-provisions for trainer-granted client access (future slice), where clients consume AI against a trainer's subscription revenue.
