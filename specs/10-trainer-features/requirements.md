...
| 8   | UI attribution                       | `Logged by Coach Bradley` badge on rows where `logged_by_user_id IS NOT NULL`. Trainer name from `profiles.full_name` (corrected 2026-07-06 — `display_name` doesn't exist on `profiles`)      |
