...
| 6   | Exercise create         | **Changed from V2.** Move from full-screen route at `(app)/exercises/create.tsx` to `<BottomSheet>` opened from the Train > Exercises segment's `+ Create` action. Sheet content per `design-source/screens/create-exercise.jsx`.                            |
