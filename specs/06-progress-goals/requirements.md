...
| 2   | TodayHero rings         | Move (`$primary`) · Train (`$ember`) · Fuel (`$gold`). All three are DAILY and reset at the user's local midnight. Move = HealthKit steps vs the user's Steps-habit target (fallback 10k). Train = HealthKit active energy vs a daily kcal goal. Fuel = daily kcal eaten vs target (gates on M9; until then shows `--` + 0% fill). **Revised 2026-07-28** — Train was weekly lifted volume vs a flat 20 t target: the only non-daily ring, and effectively uncloseable (a heavy 8,960 kg session read 45%). Weekly volume moved to the Home weekly-volume card and the You-tab VolumeStats, which already carried it. |
