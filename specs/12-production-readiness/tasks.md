...
- [ ] **T-12.13.2** Drizzle migration: `CREATE UNIQUE INDEX user_subscriptions_external_sub_uq ON user_subscriptions (external_subscription_id) WHERE external_subscription_id IS NOT NULL;` (partial — the column is nullable for legacy/manual rows). This is the load-bearing DB-level half of the receipt-replay defence — the `grantIosSubscription` first-grant-race serialisation relies on the unique violation it raises. Verify via integration test that two concurrent grants with the same `external_subscription_id` from different users return distinct GrantResult statuses (one `granted`, one `owned_by_other_user`), and that the `owned_by_other_user` path leaves BOTH users' rows untouched (the attacker's request must not cancel the caller's existing active sub).
