...
| Android                   | `HealthConnectAdapter` | Read steps, active calories, weight, body fat, heart rate and sleep; write weight, body fat and sleep; permission request/status; provider availability                                                                                                                            | Uses `react-native-health-connect`. Android 13 and lower use the Play-installed provider; Android 14+ uses the system provider. Missing/outdated-provider state links to the Health Connect Play listing. Apple Stand Time has no Health Connect equivalent and returns zero. **Basal calories are NOT read on Android** — Play review rejected the `BasalMetabolicRate` scope under the Health Connect "Minimum Scope" policy because no screen renders the value; `getBasalCaloriesToday()` returns zero. See `specs/milestones/ANDROID-LAUNCH/HEALTH-CONNECT-DECLARATION.md` before reinstating it. |
...
The `StubHealthAdapter` from 00-guardrails remains for web/fallback targets. The
old `AndroidStubHealthAdapter` is retained only as historical/test scaffolding;
production Android selection uses `HealthConnectAdapter`. The earlier M1 design
included a `SimulatorMockHealthAdapter` that returned deterministic fixtures on
the iOS simulator; that adapter was removed in PR #38 follow-up.
...
- **Unavailable provider** — Android explains that Health Connect must be installed or updated and links to Play; web uses a generic unavailable state.
...
- **Heart-rate dashboard tile** — the latest sample is visible on the Health settings screen; a dedicated dashboard tile remains future work.
...
- **Android Health Connect** — live for the current shared `HealthPort` scope. Water and dietary-energy methods proposed by this revision still require a future port extension on both platforms.
