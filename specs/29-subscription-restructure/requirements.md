...
| D12 | Premium HOLDS at £12.99/mo. The annual discount deepens to ~30 %.                                                                                                                       |
...
**AC 2.1** A per-user **daily cost budget** per tier, summed across every AI endpoint,
weighted by that endpoint's cost. ⚠ Weighted by COST, not call count: the endpoints
...
outage.
**AC 2.3** Budget sizing rule: **`max(4 × typical daily cost, 33 % of net daily
revenue)`, hard-capped at 50 % of net.** The floor is in real usage, the cap is in
margin. No bespoke per-tier percentages.
...
| C2  | **Measure the six estimated unit costs** from `ai_usage_log` or the AWS bill. 7 of 9 are derived, not measured, and the pooled budget is priced off them.                                                                                                                                                                                                                                                                | Phase 1 sizing |
