...
- **AC10:** Accepted enquiry means permission to continue discussing, never consent or access. Connection requires fresh affirmative athlete consent. New/client-origin requests create/reuse a client-initiated pending request, which the coach subsequently accepts under the existing seat lock. An existing trainer-origin pending invitation instead follows existing athlete acceptance with fresh consent and the active-seat lock; its direction is preserved. Blocked, expired or no-longer-accepting races must fail without granting access.
