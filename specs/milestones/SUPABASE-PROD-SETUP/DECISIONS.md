...
| 3 | Prod plan | **Pro + PITR 7-day** | Never pauses, daily backups, to-the-minute recovery for a launch DB. |
| 4 | Prod compute | **Small** | Minimum tier that supports PITR; comfortable for early launch; scales up live with no downtime. |
...
- Brief said "RLS is OFF" — actually **59/60 public tables have RLS enabled**; the repo
  moved *toward* RLS. Exposure is via `anon`/`authenticated` grants + one RLS-off table
...
- Brief said seed populates `serving_quantity` — the script *maps* it, but the committed
