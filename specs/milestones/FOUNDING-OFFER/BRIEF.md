...
offer, sold on the website and at founders' fairs and paid by bank transfer or a
Stripe Payment Link (no payment integration in this slice — payments are
recorded by hand). He also needs to know which vendor / event / partner each
subscriber came from so that later vendor deals can be settled. Today nothing in
the codebase can do either: there is no admin surface (the web `/org-admin` page
...
| D2  | Payment recording                                         | Manual. Admin records email, tier, amount, method (`bank_transfer` / `stripe_link` / `card_in_person` / `other`), reference. No Stripe webhook.                                                                                                                                                                                                                                                                                                                                             |
