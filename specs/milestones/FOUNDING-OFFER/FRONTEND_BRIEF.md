...
- Founding-offer section on Home + `/founding` landing route: the two consumer prices, the coach line, "X of 200 places" (number from `import.meta.env.VITE_FOUNDING_SEATS_USED` at build time for v1 — a live public counter endpoint is a follow-up), what the money funds, how to get a place in person or by emailing the monitored address, how access is switched on after signing up with and confirming the payment email, the 90-day redemption deadline, 14-day cancellation acknowledgement text, and T&Cs link. Payment is out of band; the page carries no bank details or payment buttons. Payment Links may be sent privately only. Copy: sentence case, no emojis, confident; the birthday line is one sentence, the body is about the product.
