...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Single Supabase (current) | ⏯ Sticking with **one** Supabase project for cost reasons. Workflow design is already environment-scoped, so when the project moves off free tier the same workflow files apply — only secret values diverge. `supabase/README.md` describes the single-project state as current with a documented "Upgrade path" to two projects. |
