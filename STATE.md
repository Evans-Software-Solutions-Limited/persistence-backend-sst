...
- **⚠ NEEDS FROM BRAD (prereqs — stubbed, not blocking):** (a) create Sentry project(s) + DSNs; backend → `SENTRY_DSN` GitHub Environment secret (staging/Production); mobile → `EXPO_PUBLIC_SENTRY_DSN` per `eas.json` profile. (b) source-map upload → `SENTRY_ORG`/`SENTRY_PROJECT` + `SENTRY_AUTH_TOKEN` EAS secret (auth token NOT committed). (c) **CONFIRM the privacy data-type list** (flagged in #235: SensitiveInfo for gender folded into Health — confirm; PerformanceData included because traces=0.1 — drop+set traces 0 if unwanted). (d) register privacy-policy URL `https://persistence.evans-software-solutions.com/privacy` in App Store Connect + fill the App Privacy label to match. Until DSNs are set, Sentry is OFF everywhere and the apps run byte-identically.
