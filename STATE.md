...
### 2026-09-23 — Mealprint PR preparation

Prepared isolated `codex/mealprint-preference-trust` from current origin/main,
preserving unrelated work. Production read confirmed no pescatarian pattern;
the saved natural-language category dislike bypassed literal filtering.
Added bounded fish/category exclusion and conservative tinned-tuna sandwich
exception support; no production preference mutations. Inspector Brad resolved
four findings (relative presets, negative lists, SQL cap diversity, brand-name
ingredient confusion) and returned CLEAN. Final focused checks: 595 backend,
199 mobile tests, typecheck 9/9 and lint 6/6. Evidence and live/device limits are
in `docs/mealprint-trust-fix-verification-2026-09-23.md`. No native build or deploy.

### 2026-09-23 — Mealprint dislike enforcement (local, not deployed)

Fixed literal-only dislike filtering: `fish` now excludes fish AND shellfish,
including sea bass, prawns, crab and mussels, per Brad's explicit expectation.
Category rules use known tags and existing name axes; specific species dislikes
remain specific. Generated meal titles are also checked. Swap verification now
rejects excluded items instead of ignoring the final failed avoidance verdict.
Allergen constraints retain their existing separate semantics.

Evidence: final focused Mealprint suite 496 tests passes, core typecheck passes;
regression tests fail with the fixes removed (16 original cases, 4 title/swap
cases, 11 fish-includes-shellfish cases). Full repository test task 21/21,
initial full typecheck 9/9, final lint 6/6 and formatting pass; non-mobile build
12/12. Broader test run overlapped the final vocabulary adjustment, so the
496-test final focused run is the evidence for that adjustment. No native build,
deployment, commit or PR. Existing unrelated Apple test/STATE edits preserved.
Notification skill `slack-progress-updates` remains unavailable; no ping sent.

Brad deferred a redesign, then requested optional feedback before meal swaps.
Implemented for draft and accepted plans: presets/free text, explicit generate
or cancel, feedback retained on failure, busy guards and stale-form cleanup.
Actual component web screenshots are in `docs/evidence/mealprint-swap/`; native
keyboard/sheet interaction remains unverified. Full mobile suite: 536 suites /
6,972 tests passed, followed by final lifecycle coverage run (48 tests; draft
container 92.30% branches, Today container 90.62%) and request/error-copy tests
(176 passed). Test-only QueryClient GC timers were disabled in affected helpers
to prevent the completed focused runs waiting on five-minute timers.

Brad then reported “something quick and chicken based” returning prawns. Added
bounded common-protein request parsing, culinary prompt priority, requested-food
SQL ordering and pre-cap retention, and resolved-ingredient verification for
suggestions/swaps. A single requested protein group must supply >=5g and >=50%
of plate protein; a title, chicken flavouring or token garnish cannot pass.
Saved avoidances win; unavailable matching ingredients return no_candidates
before paid inference. Unsupported prose/preparation times remain prompt-only;
day-plan requests retain prompt-only semantics to avoid imposing dinner requests
on every breakfast. Final helper coverage is 100%; focused backend run passed
555 tests, then four additional negation regressions passed (24 helper tests).
Revert checks prove four guidance/ranking regressions fail without enforcement.

Live synthetic model runner: `scratchpad/mealprint-guidance/live-smoke.ts`.
Import-only check passes; actual model calls were NOT run because ess-dev SSO
expired and refresh failed. User was asked to refresh access. No deployed demo
claim: backend deployment and an authenticated live walkthrough remain needed;
mobile release/build remains Brad-owned.

Final broad gate reruns encountered shared coverage-temp-file collision and
host-contention timeouts in unrelated invitation/subscription helper tests.
Stopped only verified orphan workers from this task and reran with two workers.
These retries are recorded rather than counted as a clean first-pass result.
The last parser refinement also needed explicit boolean annotations for TS7022.
Final bounded backend run: 5,320 pass / 2 timeout failures in unchanged
adminMarketingHandler.integration tests; isolated admin rerun passes 78/78
assertions (single-file global coverage threshold failure is expected). Keep
this flaky-run evidence visible. Final non-mobile build passes 12/12 after the
boolean annotations. Consolidated evidence and release boundaries:
`docs/mealprint-trust-fix-verification-2026-09-23.md`.

Coach nutrition audit confirms client target setting/calculator and aggregate
calorie adherence exist; manual coach meal/plan sharing (spec 38 N3–N8) remains
unimplemented. Existing personal recipe imports support photo, URL and pasted
ingredients/method; whole-plan PDF/image/notes import is not implemented. Review:
`docs/coach-nutrition-capability-review-2026-09-23.md`.
