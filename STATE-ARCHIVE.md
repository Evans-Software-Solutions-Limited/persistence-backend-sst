# Session archive · persistence-backend-sst

Sessions older than the four most recent, moved out of `STATE.md` on 2026-07-27
so the live ledger stays readable. **Nothing was deleted** — this is the full
historical log, newest first, from 2026-07-24 back to 2026-07-05.

Read `STATE.md` first. Come here only when you need the history behind a specific
decision, PR or bug. If something in here contradicts `STATE.md`, `STATE.md` wins
— these entries are frozen at the time they were written and were NOT updated as
later sessions changed things.
...
- **AnyGym verdict: brief-ready, NOT build-ready.** `specs/21-adaptive-workout-ai/` holds only the Phase-0 BRIEF (no triplet); designs are ready per Brad but live off-repo (`~/Downloads/design_handoff_site_and_anygym/` — commit them somewhere durable); 2 data-model decisions open (parent↔variation linkage, saved-gyms — recommended defaults in the brief); tier-gating divergence GTM-brief-vs-pricing-spec must resolve to the pricing spec (hard Premium+ gate; NB the 2026-07-17 site-copy revision dropped the free AI taster — Free's cap is "3 Custom Workouts" — so taster existence is an open Brad call for both AnyGym and AnyMeal). Full sense-check appended to the spec-21 BRIEF. Next step = one authoring session for the triplet.
- **spec-26 AnyMeal authored** (`specs/26-anymeal-meal-planning/` requirements + design + tasks): Premium+ AI meal planning — fill-my-macros suggestions (P1 hook), day-plan generation (P2), week+shopping list (P3, needs the async-job infra shared with spec-20). Core design mirrors AnyGym: **candidate-constrained composition** — deterministic layer owns all macro numbers (verified `foods` rows) + allergen/avoidance filtering enforced deterministically twice; model composes only; draft-confirm always. UK-first localisation (curated OFF seed + locale tags + "hard to find" feedback). New tables: `nutrition_preferences`, `meal_plans`, `meal_plan_meals`, + `foods` tag columns via seed-ETL extension. Log keeps its 4 slots (plans map onto them — no hot-table migration). Market research 2026-07-24: Eat This Much = the generator benchmark; MFP Premium+ ($24.99/mo) validates tier + is US-locked on groceries; MacroFactor has NO planning; allergen unreliability is the industry's weak point → our deterministic posture is the differentiator.
- **⚠ Open Brad checkpoints (spec-26 § Open checkpoints):** AnyMeal branding/trademark; suggestion-tier + taster; ceiling numbers; marketing-site Premium+ copy; allergen chip vocabulary + disclaimer sign-off; P3 timing. **Shared prerequisite flagged:** M19-P0 `premium_plus` tier restructure is needed by BOTH spec-21 and spec-26 — build it once, first.
- Claude Design Brief for AnyMeal screens handed to Brad in chat (not committed, per feedback_setup_briefs_as_chat_copy).
- **SAME-DAY FOLLOW-UP (Brad decisions, 2026-07-24 — ALL spec-26 + spec-21 checkpoints CLOSED):** (1) **Premium+ REPRICED £19.99 → £29.99/mo · £299.99/yr** on pricing research (AI-programming apps alone at $29.99–34.99/mo — Ladder/Juggernaut; MFP meal-planning tier $24.99; Premium+ now spans tracking+AnyGym+AnyMeal). Live site updated in the same PR (`Pricing.tsx` price + "Flagship · AnyGym + AnyMeal" ribbon + AnyMeal feat + FAQ; `Support.tsx`; SEO line) — done while Premium+ is non-purchasable (no ASC products/catalog row → zero grandfathering). Rationale ledgered in `WEBSITE_PRICING_SPEC.md § 1`. (2) **NO taster, adaptive-suite-wide** — hard Premium+ gate; promos via RevenueCat promotional entitlements only. (3) **AnyMeal + AnyGym branding CONFIRMED** — ⚠ web check found **AnyGym Ltd (UK gym-access company, any-gym.com, est. 2016)** 🟠 and an "AnyMeal — Food Diary" App Store app 🟡; no registered marks surfaced; formal UK IPO search still advised before marketing spend (findings in `WEBSITE_PRICING_SPEC.md § 5`; fallback name "Kit-Aware"). (4) Ceilings 20/5/10 locked. (5) Allergen set + disclaimer signed off. (6) **P3 (week plans + shopping list) IS v1 scope** → async-job infra in-scope, coordinate with spec-20. (7) spec-21 data-model defaults CONFIRMED (parent_workout_id linkage + saved_gyms). (8) **AnyGym design pass is running in parallel (Brad-driven)** — the executing/authoring agent gets the design brief IN CHAT at session start; do NOT block on committed design files, ask if missing.
- **RENAME (Brad, 2026-07-24, later same session): AnyGym → "Loadout", AnyMeal → "Mealprint" — both old names RETIRED** after Brad asked for unique names with zero legal budget (AnyGym Ltd + AnyMeal-app collisions). Full rename executed: site (`Home.tsx` incl. section id `#loadout`, `Pricing.tsx`, `Support.tsx`, nav/footer, CSS classes `anygym*`→`adapt*`), tests (now assert old names never resurface, flagship asserts 29.99 + both new names), spec dir `26-anymeal-*`→`26-mealprint-meal-planning` (+ `mealprint_ingredient_feedback` table name), spec-21 rename banner (design-handoff assets still SAY AnyGym — read as Loadout), pricing spec §5 decision record. **NEW: Mealprint SECTION added to the Home page** (sibling of the Loadout block — steps + plan mock + pricing CTA; there was NO specced site section for the meal feature before, the site predates spec-26). Brad's follow-ups: free UK IPO word-mark searches (classes 9/41/42/44) + optional £170 self-filing; grab mealprint domains/handles.

**2026-07-23 (BRIEF-7 device-QA sweep — ALL 20 bugs, ONE PR). PR #303 MERGED to `main` (squash `006c0e0`); CI all-green + Inspector-Brad-local clean @ `d79f0c9` (full whole-PR audit). Brad authorised + I merged. Branch `claude/brief-7-device-qa-sweep` deleted; #302 (QA-9-only) closed + folded in.**

- **Scope:** all 20 device-QA bugs (QA-1..QA-20) from Brad's 2026-07-22 TestFlight testing, fixed in ONE PR at Brad's request (overrode one-PR-per-workstream). Verified each hypothesis in code first (5 parallel Explore agents); several brief hypotheses were wrong.
- **Owner design calls (AskUserQuestion):** QA-10/11 = MINIMAL not remodel — root cause was a stray legacy per-row `assignment_notifications` Postgres trigger, NOT the data model (dropped it + one app-level notify + `getTodaysTraining` date filter). QA-14 = cache-invalidation not realtime (domain lists use custom `useCachedResource`+SQLite, NOT TanStack Query — Brad's premise corrected). QA-5/7 = follow user's preferred volume unit, **litres default** (cups only for imperial), display-only conversion, keep both water targets editable, NO schema/migration; coach sets a client's water target in the CLIENT's unit (exposed client `preferredUnits` on ClientDetail).
- **Fixes:** A(QA-1..4) backend derives Gym(sessions)+Calories(within-tolerance) per-day into Home grid via `getHabitCompletions?includeDerived` + steps/sleep bridge-on-write + derived rows kept out of SQLite/sync and held in in-memory ref across reload(). QA-6 shared habit UI locked coach-assigned habits for the coach too (`locked = config.locked && !isCoach`) + Saved toast. QA-9 bounded auto-retry in useProfilePage + drawer error/retry (deferred #296 follow-up). QA-12/13 drain sync queue before library GET + focus-refetch. QA-14 fixed useCachedResource in-flight latch (thrown fetch stranded inFlightRef→all refresh() no-op) + roster-slot invalidation on accept/decline/invite/deep-link. QA-18 past-due-only count + ADHERENCE_MIN_SAMPLE=3 grace→null "Not enough data yet". QA-19/20 Fuel day-nav (chevrons + calendar modal, past+today only) + selected day threaded into all log flows. QA-8/15/16/17 hitSlop / name-placeholder / dead-nav / mode-switch reorder.
- **Gates:** typecheck 8/8, lint 0-err, build 13/13, backend **2680 tests/98.46% cov (≥90% held)**, mobile **444 suites/4992 tests**. IB found+fixed one real 🟠 mid-sweep (derived tiles blanking on grid toggle); final whole-PR audit clean, 3 inert 🔵.
- **⚠ OPEN (Brad's):** (1) **MANUAL PROD migration** `supabase/migrations/20260722120000_drop_assignment_notifications_trigger.sql` — MUST apply WITH the prod deploy or QA-10 flood persists in prod (staging auto-applied on merge). (2) **Device-verify** on next EAS build (habit auto-complete, 1-notif programme assign, Fuel day-nav, clients-list refresh). (3) inert IB 🔵: cross-user derived-row ref capture in useGetHabits (can't mistick — per-user goalIds).
- **LESSON:** local `bun run typecheck` is turbo-CACHED and masked a real TS2741 (a test fixture missing a new field I forgot to stage) → cost a red CI run. Before pushing a big branch run `TURBO_FORCE=true bun run typecheck` + prettier on the COMMITTED tree (stash untracked) so local matches CI's cold checkout.
- See `memory/project_device_qa_bugs_brief7.md` + `specs/milestones/GO-LIVE-FINAL/BRIEF-7-device-qa-bugs.md`.

**2026-07-20 (Coach↔client OFFBOARDING — spec 25, go-live blocker). Single PR off `main` `59d0354`; gates green + Inspector-Brad-local clean (1🟡+1🟢 fixed); Brad pre-authorised merge on green CI + coverage + IB. Branch `claude/coach-client-offboarding`.**

- **Why:** there was NO way to end an active coach↔client relationship (only `terminated` writes were guarded to `status='pending'` = declining a pending invite). Coach access was effectively permanent. Net-new (legacy never had it).
- **Decisions (Brad, 2026-07-20):** bidirectional (`DELETE /trainers/me/clients/:clientId` coach-removes + `DELETE /clients/me/relationships/:relationshipId` client-leaves); **soft-end** (`status='terminated'`+`end_date`, row kept, seat freed, re-invitable); **HYBRID teardown** — DELETE coach→client workout+programme assignments, KEEP habits+goals (honors **spec-18 locked decision 6**: habits transfer to client via the status-computed edit-lock — lifts automatically, NO teardown code). Resume = assignments fresh, habits re-lock, notes/history carry over. AI access = tier-derived no-op.
- **Backend:** `endCoachClientRelationship.ts` (one tx: conditional soft-end guard → delete programmeAssignments [cascades occurrences] → delete ad-hoc workoutAssignments → audit `relationship_terminated`), 2 endpoint handlers, `coachClientOffboardingRoutes` sub-app nested in `trainersOnBehalfRoutes` (root chain hit **TS2589** — depth ceiling), best-effort `notifyRelationshipEnded`. 2 idempotent enum migrations (`20260720120000` action_type, `20260720120100` notification_type). **B7 seat/roster + B8 re-invite = already handled by existing code, verified no-change.**
- **Health-data guard fix (folded in):** migrated `trainersClientBodyTrendHandler` + `trainersClientActiveProgrammeGetHandler` off inline guards onto shared `assertTrainerCanActForClient` (adds role + trainer-soft-delete checks). Audit found coaches see only curated metrics; raw HealthKit (sleep/steps/HR) is self-only.
- **Mobile:** kebab→"Remove client" (was dead), "Leave coach" on Profile active-trainer rows, ports+adapter, SQLite invalidation + refocus-refresh. IB 🟡 fix: `onDismiss` resets the double-tap guard (Android tap-outside left the button dead).
- **Gates (root):** prettier · typecheck 8/8 · lint 0-err · build 13/13 · test:unit **core 2557 / mobile 4765 / web 13**, coverage ≥90%.
- ⚠️ **PROD migration apply = MANUAL** (both enum ADD VALUEs). ⚠️ **NOT device-verified** (rides next EAS build). ⚠️ Go-live flags surfaced to Brad (not bugs): **no explicit consent construct** for coach health-data sharing (implicit in active relationship); **no read-audit** of coach health-data views.
- See `memory/project_coach_client_offboarding.md` + `specs/25-coach-client-offboarding/`.

**2026-07-19 (hide public sharing for v1 launch — Apple Guideline 1.2 UGC de-risk). PR [#269](https://github.com/Evans-Software-Solutions-Limited/persistence-backend-sst/pull/269) raised off `main` `7b833cf`; gates green + Inspector-Brad-local clean @ `d4f9718`; awaiting Brad's merge.**

- **Why:** the app exposed stranger-facing public sharing with NO discovery UI and NO moderation (no report/flag/block). Removed the user-facing public surface for v1; public sharing returns WITH moderation later. Brad authorised the port-fidelity deviation (intentional reduction). NO backend/schema/migration change — `"public"` stays valid server-side; friends + coach→client + system/template (`createdBy IS NULL`) workouts untouched.
- **Changes (packages/mobile, presentational only):** (1) `WorkoutFormBody` — dropped "Public" from selectable `VISIBILITY_OPTIONS` (Private/Friends only); kept the type-union + icon "public" key; editing an ALREADY-public workout shows a **disabled read-only "Public" chip** (one-way off-public). (2) `EditProfilePresenter` — public-profile Switch no longer rendered (import removed; props kept on type/container wiring for a moderated re-launch). (3) `PrivacySettingsPresenter` — whole **Profile Visibility** section hidden (lone "Private" row = no real choice); Legal + Delete Account intact; removed orphaned `IconCheck`/`optionSelected`/`optionDescription`. (4) `ProfileDrawerPresenter` — Privacy row subtitle `"Profile visibility · Data & policy"` → `"Data & policy · Account"`.
- ⚠️ **LAUNCH DATA-OP REQUIRED (Brad's call, NOT in this PR):** already-public *profiles* (legacy app / earlier build) have no in-app off-ramp now. Brad chose to ship fully-hidden + run a **one-time `UPDATE profiles SET is_profile_public=false` (all users) at launch** so nobody is stranded publicly discoverable. Run against STAGING + PROD before submission.
- **Gates (root):** prettier clean · typecheck 8/8 · lint 0-errors · mobile `test:unit` **430 suites / 4686 tests, coverage ≥90%**. New/updated tests: WorkoutCreator (no Public chip), WorkoutEditor (read-only public edge case + one-way-off), EditProfile (switch never renders), PrivacySettings (section/options gone, Legal+Delete present). ⚠ **`@persistence/web#build` fails LOCALLY** (missing `@fontsource-variable/geist` in local `node_modules`) — PRE-EXISTING on clean `main`, mobile-only PR, CI (fresh install) unaffected.
- ⚠️ **NOT device-verified** — rides the next EAS dev build: picker shows only Private/Friends; no Public Profile toggle; Privacy Settings has no visibility section; seeded/legacy public workout shows the read-only Public chip.
- Slack/ntfy ping attempted at handoff (see notification rule).

**2026-07-17 (Train-hub pre-launch tidy — GO-LIVE-FINAL/BRIEF-5 Phase A). PR #264 raised off `main` `1fae2af`; committed + gates green + Inspector-Brad-local clean; merging on green per Brad. This is the LAST app-code before the launch build.**

- **A1 — Goals HIDDEN for launch (decision C).** Goals were inert/half-shipped and the athlete `<GoalSheet>` was NEVER mounted in the app tree (only its own test rendered it) — so the whole goal UI stack was already dead in-app. Removed the Goals surface from `TrainOverviewContainer`/`TrainOverviewPresenter`; DELETED the genuinely-dead `GoalsSection.tsx` (+ test + barrel), the orphaned untested `useGetGoals.ts`, and the hardcoded-zero backend `dashboardRepository.getActiveGoalsWithProgress` + `DashboardData.activeGoals` (no consumer) + its mobile model/fixture mirror. **PARKED** (kept + still test-covered) the reusable building blocks for the future make-real spec: `GoalSheet.tsx`, composite `GoalCard.tsx`, `goal-sheet` store, `goals.command.ts`. Backend `/goals` handlers + DB + the `Goal` model + SQLite storage slots UNTOUCHED (shared plumbing). **Brad's steer: goals WILL be re-added (a future spec) — hence park, not full-delete.**
- **A2 — Training segment GATED on an active human coach.** `TrainHubContainer` derives `hasCoach` from the EXISTING `useClientRelationships("active")` hook — **NO backend change / NO migration** (the endpoint already encodes `clientId=me AND status='active' AND isAiTrainer=false`; the brief's suggested new endpoint was unnecessary). No-coach athletes default to Workouts + the Training tab is hidden + the redirect persists (changes the deliberate M16 "lead with Training" default for no-coach athletes — **confirmed with Brad**). Gate uses an `effectiveSegment` in-render coercion + a persist effect.
- **Inspector-Brad found a real 🟠 High on the first pass** (now fixed + regression-tested): the network-only signal starts `data=[] isLoading=true`, and reading that as "no coach" made the gate coerce a coached athlete to Workouts AND **persist** it → permanent demotion off Training. Fix: `showTraining = hasCoach || (coachLoading && segment === "Training")` so Training is held during the round-trip for anyone already on it; coercion/persist only fire once resolved-no-coach. New test drives `data=[]+isLoading=true+segment="Training"` and asserts no demotion (fails against the naive impl). Re-reviewed **clean @ f18e4e7** (code sha; docs commits added on top don't change the reviewed code).
- **Gates (root):** prettier clean · typecheck 8/8 · lint 0 errors · build 13/13 · **core 2518 + mobile 4670 tests, coverage ≥90%.** Two judgment calls flagged in the PR: park-not-delete the goal machinery; reuse the existing hook vs a new endpoint.
- **Docs updated this session + folded into #264:** this ledger entry + `app-store-tracker.html` (header stamp → v1.2.1 / `1fae2af` / 17 Jul; "Athlete Training page" note = goals hidden + Training gated; "Programs tab" note refreshed for the #260 unified-hub slice) + the tracker Artifact republished (same URL). ⚠ **NOT device-verified** — rides the next EAS dev build alongside the coach-tooling (#260) verify.
- Slack/ntfy pings still unavailable this session (Bash `curl` sandbox-denied + Slack connector not authorized) → none sent.

**2026-07-17 (marketing WEBSITE — executes GO-LIVE-FINAL/BRIEF-4-website) — built + **PR #261 MERGED** (squash → `main` 20:04Z; all 5 PR-Checks green, MERGEABLE/CLEAN; Inspector-Brad-local clean @ `cf13aae`). The public marketing site + Support-URL App-Store gap are DONE.**

- **Shipped into `packages/web`** (Vite/React/Tailwind v4; scoped `.mkt` warm-dark editorial theme + a derived light variant; self-hosted Geist + Instrument Serif): **`/` landing** (hero, feature marquee, three pillars, AnyGym, coach, app-store, footer), **`/pricing`** (athlete Free/Premium/Premium+ + 3 coach tiers + Teams + monthly/annual toggle + FAQ), **`/support`** (contact + quick links + FAQ — **closes the App-Store-required Support-URL gap**). `/privacy`+`/terms` restyled into the same shell (content unchanged, paths stable); `/login` left a stub. Default **dark** + minimalist nav theme-toggle (`ThemeProvider defaultTheme="dark"`, affects the whole web app).
- **Hero + coach phone mocks MIRROR the real app** — pure-CSS screens rebuilt from the actual `packages/mobile` presenters (HomePresenter+habits / ActiveSession / Fuel; CoachHome / ClientsList / ClientDetail incl. the flagship **AI weekly summary**). Auto-crossfade (keep cycling on hover), reduced-motion-safe, `aria-hidden`. `config.heroScreenshot` can drop a real capture into the frame but isn't needed.
- **HARD EXCLUSION honoured:** NO founding-waitlist / early-bird-discount anywhere (Brad reviews discount mechanics separately). App-Store CTAs are non-linking **"Coming to the App Store"** (`packages/web/src/marketing/config.ts` › `appStore.available=false`; flip to go live + unlock an iTunes-lookup rating badge). Fabricated 5-star row dropped; hero leads offline-first.
- **Branding/SEO:** real ring-less Persistence logo (mobile `apple-touch-icon`) in nav/footer + full favicon set + `site.webmanifest` (PWA `manifest:false`); shadcn brand token reconciled orange→cyan; per-route `useSeo` title/meta/canonical, JSON-LD (Organization + MobileApplication), `robots.txt`, `sitemap.xml`, Nottingham/UK keywords. OG image = logo tile (a designed 1200×630 banner is an optional follow-up). ⚠ **OPEN Brad decision (parked, cosmetic):** brand/logo is BLUE `#1E9BFF` but the site accent is prototype CYAN `#4FE3F0` — keep cyan or retune the accent to blue.
- Fixed an unformatted Brad-dropped `public/site.webmanifest` that would have failed the tree-wide `prettier --check .` in CI. Local gates all green (prettier whole-repo / typecheck 8·8 / lint 6·6 / build 13·13 / test 19·19). Repo is **PUBLIC** at merge time (Brad accepts; going private soon) — no secrets in the committed docs (scanned).
- **Website launch line-item DONE.** Supersedes the earlier "`/` is a placeholder + Support URL missing" finding. Competitor sense-check (Brad ask) vs Trainerize (coach-first B2B) + Hevy/Strong/Fitbod → positioned Persistence as "athlete tracking + built-in coaching, offline-first, UK".

**2026-07-17 (coach-tooling IA slice — the HARD pre-build blocker) — spec-24 authored + built end-to-end, gates green, IB-local clean, **PR #260 MERGED** (squash → `origin/main` HEAD `6156380`; all 5 CI checks green pre-merge, MERGEABLE/CLEAN). The pre-build blocker is now CODE-COMPLETE; **device-verify is the only remaining gate before the next EAS build.****
