...
function get(
  ...
  return new Request(
    `http://localhost/trainers/me/clients/${clientId}/measurements${query}`,
    { method: "GET", headers },
  );
}

describe("trainersMeListClientMeasurementsHandler", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("200 returns the client's measurements", async () => {
    const { trainersMeListClientMeasurementsHandler } =
      await import("../trainersMeListClientMeasurementsHandler");
    const res = await trainersMeListClientMeasurementsHandler.handle(
      get("client-1"),
